...
# USER SETTINGS
# -----------------------------
# Read from the current notebook folder
INPUT_DIR = Path.cwd()

# Optional fallback only if it exists on the machine
...
FILE_MAP = {
    ...
    "SR500":  "04_SR500.csv",
    "SR550":  "05_SR550.csv",
    "SA1100": "06_SA1100.csv",
}

PLOT_ORDER = ["AS", "SR400", "SR450", "SR500", "SR550", "SA1100"]

# Restrained cleanup adapted to your latest LPBF 3 wt.% Ni set
FIT_OVERRIDES = {
    "AS":     {"trim_high": 0, "trim_low": 0, "use_robust_loss": False},
    "SR400":  {"trim_high": 0, "trim_low": 0, "use_robust_loss": False},
    "SR450":  {"trim_high": 0, "trim_low": 2, "use_robust_loss": False},
    "SR500":  {"trim_high": 0, "trim_low": 0, "use_robust_loss": False},
    "SR550":  {"trim_high": 0, "trim_low": 1, "use_robust_loss": False},
...
# Always save locally
OUT_DIR = Path.cwd() / "EIS_1RQ_weighting_comparison_final"
...
def resolve_input_file(filename: str):
...
def read_eis_csv(csv_file: Path, trim_high: int, trim_low: int):
    ...
    if start >= end:
        raise ValueError(
            f"Invalid trimming for {csv_file.name}: trim_high={trim_high}, trim_low={trim_low}, n_points={len(clean)}"
        )
...
def fit_metrics(z_exp, z_fit):
    ...
    # Common modulus-relative metric for comparing all weighting schemes
...
def plot_bode(out_dir, sample_name, weighting, freq, z_exp, freq_dense, z_fit_dense):
    ...
    axes[0].set_xlim(np.min(freq_dense), np.max(freq_dense))
    ...
    axes[1].set_xlim(np.min(freq_dense), np.max(freq_dense))
...
for sample_name in PLOT_ORDER:
    fname = FILE_MAP[sample_name]
    fpath = resolve_input_file(fname)
...
comparison = comparison[["sample"] + WEIGHTING_SCHEMES + ["best_weighting_by_modulus_wRMSE", "best_modulus_wRMSE"]]
